#setwd("~/Matt/logregmod")
#setwd("//lancs/homes/45/georgem1/My Documents/DISS/logregmod")
library(rjags)
library(mvtnorm)
library("doParallel")
...
#best values brought forward from previous calibration
c1=-2.5
c2=0
v1=1
v2=0.5
...
#values 
ewoc_val<-seq(0.25,0.1,by=-0.01)
sc_ewoc1<-matrix(NA,nrow=nsims,ncol=4)
sc_ewoc2<-matrix(NA,nrow=nsims,ncol=4)
sc_ewoc3<-matrix(NA,nrow=nsims,ncol=4)
sc_ewoc4<-matrix(NA,nrow=nsims,ncol=4)
sc_ewoc5<-matrix(NA,nrow=nsims,ncol=4)
for(e1 in 1:length(ewoc_val)){
try(
  sc_ewoc1[e1,]<-colMeans(foreach(i=1:nsims, combine=rbind ) %dopar% {
    SIMpar2(i,true=s1,mean1=c1,mean2=c2,var1=v1,var2=v2,var3=v3,safe=ewoc_val[e1])
  })
)
save.image("BLRM_priorcal_ewoc.RData")
}
          
for(e1 in 1:length(ewoc_val)){
  try(
    sc_ewoc2[e1,]<-colMeans(foreach(i=1:nsims, combine=rbind ) %dopar% {
      SIMpar2(i,true=s8,mean1=c1,mean2=c2,var1=v1,var2=v2,var3=v3,safe=ewoc_val[e1])
    })
  )
  save.image("BLRM_priorcal_ewoc.RData")
}         
...
for(e1 in 1:length(ewoc_val)){
  try(
    sc_ewoc3[e1,]<-colMeans(foreach(i=1:nsims, combine=rbind ) %dopar% {
      SIMpar2(i,true=s10,mean1=c1,mean2=c2,var1=v1,var2=v2,var3=v3,safe=ewoc_val[e1])
    })
  )
  save.image("BLRM_priorcal_ewoc.RData")
}
...
for(e1 in 1:length(ewoc_val)){
  try(
    sc_ewoc4[e1,]<-colMeans(foreach(i=1:nsims, combine=rbind ) %dopar% {
      SIMpar2(i,true=s13,mean1=c1,mean2=c2,var1=v1,var2=v2,var3=v3,safe=ewoc_val[e1])
    })
  )
  save.image("BLRM_priorcal_ewoc.RData")
}
...
for(e1 in 1:length(ewoc_val)){
  try(
    sc_ewoc5[e1,]<-colMeans(foreach(i=1:nsims, combine=rbind ) %dopar% {
      SIMpar2(i,true=s14,mean1=c1,mean2=c2,var1=v1,var2=v2,var3=v3,safe=ewoc_val[e1])
    })
  )
  save.image("BLRM_priorcal_ewoc.RData")
}